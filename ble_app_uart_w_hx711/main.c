...
uint32_t read_temp()
{
    /** Returns temperature. Stalls program for 50 us **/
    uint32_t temp;
...
//    uint16_t length = sizeof(int);
    ...
    char conv_value[9];
    itoa(value, conv_value, 10);

    (void)ble_nus_data_send(&m_nus, (uint8_t *) conv_value, (uint16_t  *) 9, m_conn_handle);
    NRF_LOG_INFO("ADC sent measuremement %d", value);
}
...
{    
    ...
    {
        send_data(value);
        // TODO: Send temperature value with threshold reading
        if (value < threshold) {
            // if alarm_high, Green off, Red on
...
//         -  TX buffer is full*/
//        (void)ble_nus_data_send(&m_nus, (uint8_t *) conv_value, 9, m_conn_handle);
...
int main(void)
{
    ...
    // On start, wait for button press. Yellow LED on
    bsp_board_led_on(BSP_BOARD_LED_1);
    
    // When strap is tensioned. User press button. Yellow LED off, Green on.
    while(!bsp_board_button_state_get(1)) {}
    bsp_board_led_off(BSP_BOARD_LED_1);
    bsp_board_led_on(BSP_BOARD_LED_2);

    // Start execution.
    NRF_LOG_INFO("Debug logging for UART started.");
    advertising_start();
