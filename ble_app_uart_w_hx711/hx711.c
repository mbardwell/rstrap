...
#define BUFFER_LEN 5
#define DEMO
uint32_t data_buffer[BUFFER_LEN+1];
uint8_t counter = 0;
static bool first_pass = true;
uint32_t threshold = 17000;
...
static void hx711_sample()
{
    ...
    if (counter < BUFFER_LEN) {
        data_buffer[counter] = convert_sample(m_sample.value);
        NRF_LOG_INFO("databuffer: %d", data_buffer[counter]);  
        counter++;
    }

    if (m_sample.count == ADC_RES && counter >= BUFFER_LEN)
    {
        uint32_t average = 0;
        uint32_t sum = 0;
        ...
        average = sum/BUFFER_LEN;
        if (first_pass) {
            threshold = average * 0.9;
            first_pass = false;
            NRF_LOG_INFO("*************************Threshold: %d", threshold);
        }

        #ifdef DEMO
            m_evt_handler(DATA_READY, average);
        #else
            if (average < threshold) {
                m_evt_handler(DATA_READY, ALARM_HIGH);
            }
            else {
                m_evt_handler(DATA_READY, ALARM_LOW);
            }
        #endif
        counter = 0;
    }
